...
dc.leafletChoroplethChart = function(parent, chartGroup) {
  ...
  var processFeatures = function (feature, layer) {
    ...
    if (v && v.d) {
      ...
      if (_chart.brushOn()) {  
        ...
        layer.on("mouseover",hoverFilter);
      }
    }
  }
  ...
  var selectFilter = function(e) {
    ...
    dc.events.trigger(function () {
      ...
      _chart.filter(filter);
      console.log("_chart.filter(filter): ", _chart.filters())
      dc.redrawAll(_chart.chartGroup());
    });
  }

  //
  var hoverFilter = function(e) {
    if (!e.target) return;
    var hoverfilter = e.target.key;
    console.log("hoverFilter e: ", e)
    dc.events.trigger(function () {
      console.log("hover filter: ", hoverfilter)
      _chart.filter(hoverfilter);
      //_chart.filter(filter);
      //dc.redrawAll(_chart.chartGroup());
    });
  }
