dc.leafletChart = function(_chart) {
  ...
  _legend.update = function () {
    //console.log("in legend.update choroChart.colorDomain().apply(choroChart): ", choroChart.colorDomain().apply(choroChart))
    //console.log("minmaxEvents: ", minmaxEvents)
    //console.log("in legend.update _chart.minmaxEvents: ", _chart.minmaxEvents())
    //console.log("in legend.upate choroChart.colorDomain(): ", choroChart.colorDomain())
    //console.log("in legend.upate choroChart.colorDomain()[0]: ", choroChart.colorDomain()[0])
    //console.log("eventRange in legend.update: ", eventRange)
...
dc.leafletChoroplethChart = function(parent, chartGroup) {
  ...
  //-----------------katiRG edits--------------------
  var _colors = d3.scale.category20c().range(colorbrewer.YlGn[7]);    
  var _colorDomain = [0, colorbrewer.YlGn[7].length];  

  //modified to accept minmaxValue which is previously calculated in the choro fn
 ...
 //      if (!dc.utils.isNumber(maxValue)) return _colors(value);
      ...
      //console.log("value, minValue, maxValue, colorsLength in dc.js: ", value +", "+ minValue +", "+ maxValue +", "+ colorsLength)   
      var colorValue = Math.abs(Math.min(colorsLength - 1, Math.round((value - minValue) / denominator)));
      console.log("value, colorValue, _chart.colors()(colorValue): ", value +", "+ colorValue +", "+ _chart.colors()(colorValue))
      //var colorValue = Math.abs(Math.round((value - minValue) / denominator)) % colorsLength;
      console.log("colorbrewer.YlGn[7][colorValue]: ", colorbrewer.YlGn[7][colorValue])     
      
 //      //return _chart.colors()(colorValue);
      return colorbrewer.YlGn[7][colorValue];
  };
 ...
 var _colorAccessor = function(d, i){return i;};
  ...
  //     _colorCalculator = _;
  //     //console.log("_colorCalculator: ", _colorCalculator)
  //   return _chart;
  // };
  ...
  _chart.getColor = function(d, i){    
    ...
    //pass in min and max value previously calculated in the choro chart
  ...
  _chart.minmaxEvents = function(_) { 
    if (!arguments.length) return _minmaxEvents;
    _minmaxEvents = _;
    return _chart;
  }
